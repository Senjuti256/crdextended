...
import (
	...
	//"github.com/Senjuti256/crdextended/pkg/apis/pipeline/v1alpha1"
...
func (c *Controller) syncHandler(prun *v1alpha1.PipelineRun) (*v1alpha1.TaskRun, error) {
	...
	if createTrun {
		...
		trun, err := c.prunClient.SamplecontrollerV1alpha1().TaskRuns(prun.Namespace).Create(context.TODO(), newTaskRun(prun), metav1.CreateOptions{})
		if err != nil {
...
func (c *Controller) updatePrunStatus(prun *v1alpha1.PipelineRun, trun *v1alpha1.TaskRun) error {
	...
	p, err := c.prunClient.SamplecontrollerV1alpha1().PipelineRuns(prun.Namespace).Get(context.Background(), prun.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}

	t, err := c.prunClient.SamplecontrollerV1alpha1().TaskRuns(trun.Namespace).Get(context.Background(), trun.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	_, err = c.prunClient.SamplecontrollerV1alpha1().PipelineRuns(prun.Namespace).UpdateStatus(context.Background(), p, metav1.UpdateOptions{})
	return err
}
